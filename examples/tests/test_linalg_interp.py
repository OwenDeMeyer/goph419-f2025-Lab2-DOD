...
def test_single_rhs_gauss_seidel():
    ...
    x = gauss_iter_solve(A, b, x0=None, tol=1e-8, alg='seidel')
...
def test_inverse_computation():
    A = np.array([[4, 2],
                  [1, 3]], dtype=float)
    I = np.eye(2)
    A_inv = gauss_iter_solve(A, I, x0=None, tol=1e-8, alg='seidel')
...
def test_invalid_alg():
    ...
    with pytest.raises(ValueError):
        gauss_iter_solve(A, b, x0=None, tol=1e-8, alg='invalid')

def test_dimension_mismatch():
    A = np.eye(3)
    b = np.ones(2)
    with pytest.raises(ValueError):
        gauss_iter_solve(A, b, x0=None, tol=1e-8, alg='seidel')
